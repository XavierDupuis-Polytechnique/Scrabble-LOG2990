...
export class WordSearcher {
    ...
    getVerticalNeighbours(action: PlaceLetter): Tile[][] {
        // const startPosition = action.placement;
        let listOfWord: Tile[][] = [];

        if (this.dictionaryService.isWordInDict(action.word)) {
            for (const coordinates of this.findCoordOfLettersToPLace(action)) {
                let letterPosition = coordinates;
                let currentPosition = coordinates;
                let word: Tile[] = [];
                // if (startPosition.direction === Direction.Horizontal) {
                // letterPosition.x += coordinates.x;
                // } else {
                //     letterPosition.y += coordinates.y; //Letter position= position dans le mot a verifier
                // }
                //Iteration vers le haut
                while (this.tileIsOccupied(currentPosition.x, currentPosition.y) || this.isLetterPosition(currentPosition, letterPosition)) {
                    currentPosition.y -= 1;
                }
                currentPosition.y += 1;
                //Iteration vers le bas
                while (this.tileIsOccupied(currentPosition.x, currentPosition.y) || this.isLetterPosition(currentPosition, letterPosition)) {
                    if (this.isLetterPosition(currentPosition, letterPosition)) {
                        let tile = new Tile();
                        // if (startPosition.direction === Direction.Horizontal) {

                        tile.letterObject = this.letterCreator.createLetter(action.word[letterPosition.x - action.placement.x]);
                        // } else {
                        //     tile.letterObject = this.letterCreator.createLetter(action.word[letterPosition.y])
                        // }
                        tile.letterMultiplicator = this.grid[letterPosition.y][letterPosition.x].letterMultiplicator;
                        tile.wordMultiplicator = this.grid[letterPosition.y][letterPosition.x].wordMultiplicator;
                        word.push(tile);
                    } else {
            ...
            this.placementIsValid = true; //A verifier
    ...
    isLetterPosition(currentPosition: PlacementSetting, letterPosition: PlacementSetting) {
    ...
    findCoordOfLettersToPLace(action: PlaceLetter) {
        let x = action.placement.x;
        let y = action.placement.y;
        let coord: PlacementSetting[] = [];
        let placement: PlacementSetting = { x: 0, y: 0, direction: Direction.Horizontal };
        for (let i = 0; i < action.word.length; i++) {
            if (!this.tileIsOccupied(x, y)) {
                placement = { x: x, y: y, direction: action.placement.direction };
                coord.push(placement);
            }
